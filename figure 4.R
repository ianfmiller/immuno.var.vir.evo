...
point.func<-function(file.path,pch=16,cex=2,wval,xval,yval,zval)
{
  ...
  for (theta in thetas)
  {
    ...
    prev<-sum(out[5000,(n.immunity.categories+2):(2*n.immunity.categories+3)])/sum(out[5000,2:(4*n.immunity.categories+1)])
    y<-log10(theta)
    points(y,prev,col=colors[which(thetas==theta)],cex=cex,pch=pch)
  }
...
plot(0,0,xlim=c(-2.25,2.25),ylim=c(0,.1),xlab=expression(log[10](theta)),ylab="equilibrium prevalence",type="n",axes=F,cex.lab=2)
...
plot(0,0,xlim=c(-2.25,2.25),ylim=c(0,.1),xlab=expression(log[10](theta)),ylab="ES virulence",type="n",axes=F,cex.lab=2)
