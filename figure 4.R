...
point.func<-function(file.path,pch=16,cex=2,wval,xval,yval,zval)
{
  ...
  setwd("~/Documents/GitHub/immuno.var.vir.evo/cluster/code")
  ...
  get.startconds()
  parms <-c(birth.rate=birth.rate,death.rate=death.rate,K=K,v1=v1,v2=v2,b1=b1,b2=b2,d1=d1,d2=d2,c1=c1,w=wval,x=xval,y=yval,z=zval,startconds)  
  out <<- ode(startconds, times, func = "derivs", parms = parms,dllname = "sirmodessDD",initfunc = "initmod", nout = 4*n.immunity.categories^2, outnames = paste("out",seq(0,4*n.immunity.categories^2-1,1),sep=""),verbose=F,method="lsoda")
  #plot.simulation(ylim=c(0,.1))
  prev<-sum(out[5000,(n.immunity.categories+2):(2*n.immunity.categories+3)])/sum(out[5000,2:(4*n.immunity.categories+1)])
  y<--2.25
  points(y,prev,col="black",cex=cex,pch=pch)
  ...
  for (alpha in alphas)
  {
    ...
    prev<-sum(out[5000,(n.immunity.categories+2):(2*n.immunity.categories+3)])/sum(out[5000,2:(4*n.immunity.categories+1)])
    y<-log10(alpha)
    points(y,prev,col=colors[which(alphas==alpha)],cex=cex,pch=pch)
  }
  ...
  prev<-sum(out[5000,(n.immunity.categories+2):(2*n.immunity.categories+3)])/sum(out[5000,2:(4*n.immunity.categories+1)])
  y<-2.25
  points(y,prev,col="grey",cex=cex,pch=pch)
}
...
plot(0,0,xlim=c(-2.25,2.25),ylim=c(0,.1),xlab=expression(log[10](alpha)),ylab="equilibrium prevalence",type="n",axes=F,cex.lab=2)
mtext("W=1 X=0 Y=0 Z=0",cex=2)
axis(1,cex.axis=1.75)
axis(2,at=seq(0,.3,.1),cex.axis=1.75)
...
plot(0,0,xlim=c(-2.25,2.25),ylim=c(0,.1),xlab=expression(log[10](alpha)),ylab="ES virulence",type="n",axes=F,cex.lab=2)
